...
const Home = () => {
  ...
  useEffect(() => {
    const handler = setTimeout(() => {
      ...
      setDebouncedLocation(filterLocation.trim().toLowerCase());
    }, 300);
  ...
  useEffect(() => {
    ...
    // Initialize Fuse only after profiles are available
    const fuse = new Fuse(profiles, {
      keys: ["name", "address.text"],
      threshold: 0.3,
    });

    if (debouncedSearch || debouncedLocation) {
      const searchQuery = `${debouncedSearch} ${debouncedLocation}`.trim();
      const results = fuse.search(searchQuery);
      setFilteredProfiles(results.map((result) => result.item));
    } else {
